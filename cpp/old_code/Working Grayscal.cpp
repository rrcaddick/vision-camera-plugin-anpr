...
namespace visioncamerapluginanpr {
  ...
  std::string processImage(AHardwareBuffer* hardwareBuffer, int width, int height) {
      ...
      int result = AHardwareBuffer_lockPlanes(hardwareBuffer, 
                                              AHARDWAREBUFFER_USAGE_CPU_READ_RARELY,
                                              -1, nullptr, &planes);
      std::string jsonResult = "{}";
      
      if (result == 0) {
        cv::Mat yImage, rotatedYImage;

        // Assuming planes.planes[0] is Y and planes.planes[1] is interleaved VU
        uint8_t* yPlane = static_cast<uint8_t*>(planes.planes[0].data);
        uint8_t* vuPlane = static_cast<uint8_t*>(planes.planes[1].data);

        // Create the Y (grayscale) image directly from the Y plane with the original width and height
        yImage = cv::Mat(height, width, CV_8UC1, yPlane, planes.planes[0].rowStride);

        // Rotate the grayscale image 90 degrees to correct orientation from landscape-left to portrait
        cv::rotate(yImage, rotatedYImage, cv::ROTATE_90_CLOCKWISE);

        std::string downloadsPath = getDownloadsPath();
        if (!imagesSaved) {
          if (!downloadsPath.empty()) {
              cv::imwrite(downloadsPath + "/rotated_grayscale.png", rotatedYImage);
              imagesSaved = true;
          } else {
        ...
        unsigned char* pixelData = rotatedYImage.data;
        int imgWidth = rotatedYImage.cols;
        int imgHeight = rotatedYImage.rows;
        int bytesPerPixel = 1;  // Grayscale image

        unsigned char buffer[height * width];

        // Call OpenALPR recognize method
        std::vector<alpr::AlprRegionOfInterest> regionsOfInterest;
        alpr::AlprResults results = g_openalprInstance->recognize(pixelData, bytesPerPixel, width, height, regionsOfInterest);
        // alpr::AlprResults results = g_openalprInstance->recognize(downloadsPath + "/rotated_grayscale.png");
        jsonResult = alpr::Alpr::toJson(results);
        return jsonResult;
  ...
  void installPlugins(jsi::Runtime& runtime) {
    ...
    auto recognisePlate = [](jsi::Runtime& runtime, const jsi::Value& thisArg, const jsi::Value* args, size_t count) -> jsi::Value {
      try {
        ...
        AHardwareBuffer* hardwareBuffer = reinterpret_cast<AHardwareBuffer*>(pointer);
        AHardwareBuffer_Planes planes;
        int result = AHardwareBuffer_lockPlanes(hardwareBuffer, 
                                                AHARDWAREBUFFER_USAGE_CPU_READ_RARELY,
                                                -1, nullptr, &planes);
        ...
        std::string jsonResult = processImage(hardwareBuffer, width, height);
